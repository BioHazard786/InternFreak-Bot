...
async def subscribe(_, message):
    ...
    if await loop.run_in_executor(executor, lambda: checkSubs(str(user_id))):
        ...
        await message.reply_text("<b><i>Successfully Subscribed</b></i>")
    else:
        await message.reply_text("<b><i>You are already Subscribed :)</b></i>")


@bot.on_message(filters.command(['unsubscribe', 'unsubscribe@internfreakbot']) & filters.private)
async def unsubscribe(_, message):
    user_id = message.from_user.id
    await loop.run_in_executor(executor, lambda: removeSubs(str(user_id)))
    await message.reply_text("<b><i>Successfully Unsubscribed :(</b></i>")
